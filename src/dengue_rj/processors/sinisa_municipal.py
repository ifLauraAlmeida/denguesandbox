"""Processamento municipal dos quatro componentes SINISA, referência 2023."""
...
def process_sinisa_municipal(
    ...
    for component, table in tables.items():
        path = output_directory / f"sinisa_{component}_indicadores_rj_2023.csv"
        table.to_csv(path, index=False)
        files.append(path)
    coverage = _build_coverage(tables)
    coverage_file = output_directory / "cobertura_saneamento_snis_sinisa_2020_2023.csv"
    _append_snis_coverage(coverage_file, coverage)
    return SinisaMunicipalProcessing(
        tuple(files), coverage_file, sum(len(table) for table in tables.values())
    )
