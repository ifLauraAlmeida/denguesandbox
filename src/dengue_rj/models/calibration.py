...
def fit_beta_incident_temporal(
    observed_incident: ArrayLike,
    population: float,
    ...
    """Ajusta beta a casos incidentes, com condições iniciais explícitas."""
    observed = np.asarray(observed_incident, dtype=float)
    if (
        observed.ndim != 1
        or len(observed) < 2
        or np.any(~np.isfinite(observed))
        ...
        raise ValueError("observed_incident deve ser vetor não negativo com ao menos 2 pontos")
    if (
        len(bounds) != 2
        or not np.isfinite(bounds).all()
        or bounds[0] < 0
    ...
    def simulate(beta: float, size: int) -> np.ndarray:
        ...
        return solve_sir(params, size - 1).new_infections

    def objective(beta: float) -> float:
        return float(np.mean((simulate(beta, train_size) - train) ** 2))

    fitted = minimize_scalar(objective, bounds=bounds, method="bounded")
    residuals = simulate(float(fitted.x), len(observed)) - observed
    train_residuals = residuals[:train_size]
    validation_residuals = residuals[train_size:]
